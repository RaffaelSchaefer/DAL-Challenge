...
migrate((db) => {
  ...
  collection.listRule = ""
  collection.viewRule = ""
  collection.createRule = "(id = @collection.contracts.asset && @collection.contracts.customers.customer_user_via_works_for.id = @request.auth.id) || @collection.dla_user.isUser = @request.auth.id"
